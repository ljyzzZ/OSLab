...
static uint32_t balloc() {
  ...
  for (int i = 0; i < BLK_NUM / 32; ++i) {
    ...
    if (byte != 0xffffffff) {
      ...
      for(int j = 0; j < 32; ++j) {
        if((byte & temp) == 0){
          bzero((uint32_t)(32 * i + j));
          byte = byte | temp;
          bwrite(&byte, 4, sb.bitmap, i * 4);
          return (uint32_t)(32 * i + j);
        }
        temp = temp << 1;
      }
    }
...
static inode_t *ilookup(inode_t *parent, const char *name, uint32_t *off, int type) {
  ...
  for (uint32_t i = 0; i < size; i += sizeof dirent) {
    ...
    if(!strcmp(dirent.name, name)) {
      inode_t* open_inode = iget(dirent.inode);
      if(off != NULL)
        *off = i;
      return open_inode;
    }
  ...
  uint32_t alloc_inode = dialloc(type);
  inode_t* created_inode = iget(alloc_inode);
  if(type == TYPE_DIR)
    idirinit(created_inode, parent);
  // create new entry for directory entry
  dirent_t created_dirent;
  created_dirent.inode = alloc_inode;
  strcpy(created_dirent.name, name);
  iwrite(parent, empty, &created_dirent, sizeof created_dirent);
...
inode_t *iopen(const char *path, int type) {
  ...
  inode_t* parent, *dest_inode;
  parent = iopen_parent(path, name);
  if(parent == NULL)
    return NULL;
  dest_inode = ilookup(parent, name, NULL, type);
  iclose(parent);
  return dest_inode;
}
...
static uint32_t iwalk(inode_t *inode, uint32_t no) {
  ...
  if (no < NINDIRECT) {
    ...
    uint32_t no_indirect;
    bread(&no_indirect, sizeof(uint32_t), inode->dinode.addrs[NDIRECT], no * sizeof(uint32_t));
    if(no_indirect == 0){
      no_indirect = balloc();
      bwrite(&no_indirect, sizeof(uint32_t), inode->dinode.addrs[NDIRECT], no * sizeof(uint32_t));
      iupdate(inode);
    }
    return no_indirect;
  }
...
int iread(inode_t *inode, uint32_t off, void *buf, uint32_t len) {
  ...
  /* uint32_t readable = (off + len) < inode->dinode.size ? (off + len) : inode->dinode.size;
  readable -= off;
  char* new_buf = (char*)buf;
  while(len > 0){
    uint32_t blk_order = off / BLK_SIZE;
    uint32_t blk_no = iwalk(inode, blk_order);
    uint32_t blk_off = off % BLK_SIZE;
    uint32_t read_size = (BLK_SIZE - blk_off) < len ? (BLK_SIZE - blk_off) : len;
    bread(new_buf, read_size, blk_no, blk_off);
    new_buf += read_size;
    off += read_size;
    len -= read_size;
  }
  ...
  if(off + len > inode->dinode.size)
    end = inode->dinode.size;
  uint32_t length = 0;
  while(start < end){
    uint32_t index= iwalk(inode, start / BLK_SIZE);
    uint32_t read_size = BLK_SIZE - (start % BLK_SIZE);
    if(read_size > end - start)
      read_size = end - start;
    bread( buf + length, read_size, index , start%BLK_SIZE);
    start += read_size;
    length += read_size;
  }
  return length;
...
int iwrite(inode_t *inode, uint32_t off, const void *buf, uint32_t len) {
  ...
  uint32_t record_len = len;
  char* new_buf = (char*)buf;
  while(len > 0){
    uint32_t blk_order = off / BLK_SIZE;
    uint32_t blk_no = iwalk(inode, blk_order);
    uint32_t blk_off = off % BLK_SIZE;
    uint32_t write_size = (BLK_SIZE - blk_off) < len ? (BLK_SIZE - blk_off) : len;
    bwrite(new_buf, write_size, blk_no, blk_off);
    new_buf += write_size;
    off += write_size;
    len -= write_size;
  }
 ...
 /*  uint32_t start = off;
  ...
  uint32_t length = 0;
  while(start < end){
    uint32_t blk_no = iwalk(inode, start / BLK_SIZE);
    uint32_t write_size = BLK_SIZE - (start % BLK_SIZE);
    write_size = MIN(write_size, end - start);
    bwrite(buf + length, write_size, blk_no, start % BLK_SIZE);
    start += write_size;
    length += write_size;
  }
...
void itrunc(inode_t *inode) {
  ...
  if(inode->dinode.addrs[NDIRECT] != 0){
    bzero(inode->dinode.addrs[NDIRECT]);
...
static int idirempty(inode_t *inode) {
  ...
  assert(inode->dinode.type == TYPE_DIR);
  dirent_t dirent;
  for(int i = 2; i < isize(inode) / sizeof(dirent); ++i){
    iread(inode, i * (sizeof dirent), &dirent, sizeof dirent);
    if(dirent.inode != 0)
      return false;
  }
  return true;
...
int iremove(const char *path) {
  ...
  inode_t* open_file;
  uint32_t off = 0;
  open_file = ilookup(ip, name, &off, 0);
  if(open_file == NULL){
    iclose(ip);
    return -1;
  }
  if((open_file->dinode.type == TYPE_DIR) && (!idirempty(open_file))){
    iclose(ip);
    iclose(open_file);
    return -1;
  }
