...
file_t *fopen(const char *path, int mode) {
  ...
  if (type == TYPE_FILE || type == TYPE_DIR) {
    ...
    if(type == TYPE_DIR && (mode & (O_TRUNC | O_WRONLY))) goto bad;
...
int fread(file_t *file, void *buf, uint32_t size) {
  ...
  {
    value = iread(file->inode, file->offset, buf, size);
    if(value >= 0)
      file->offset += value;
  }
  else if(file->type == TYPE_DEV)
    value = file->dev_op->read(buf, size);
  return value;
}
...
int fwrite(file_t *file, const void *buf, uint32_t size) {
  ...
  if(file->type == TYPE_FILE || file->type == TYPE_DIR)
  {
    value = iwrite(file->inode, file->offset, buf, size);
    if(value >= 0)
      file->offset += value;
  }
  else if(file->type == TYPE_DEV)
    value = file->dev_op->write(buf, size);
  return value;
}
...
uint32_t fseek(file_t *file, uint32_t off, int whence) {
  ...
  if (file->type == TYPE_FILE || file->type == TYPE_DIR) {
    if(whence == SEEK_SET)
      ...
      int true_off = (int)off;
      if(true_off >= 0)
        file->offset = isize(file->inode);
      else
        file->offset = (isize(file->inode) + true_off);
    }
    return file->offset;
  }
  return -1;
